...
class cstring {
  ...
  constexpr cstring(std::string_view a, std::index_sequence<N1...>,
                    std::string_view b, std::index_sequence<N2...>) noexcept
      : chars_{{a[N1]..., b[N2]..., '\0'}} {}
 ...
 public:
  ...
  constexpr explicit cstring(cstring<N1> a, cstring<N2> b) noexcept
      : cstring{a, std::make_index_sequence<N1>{}, b,
  ...
  std::basic_string<Char, Traits, Allocator> str() const {
    return {begin(), end()};
  }
...
template <std::size_t N>
constexpr bool operator==(const cstring<N>& lhs,
...
std::basic_ostream<Char, Traits>& operator<<(
    std::basic_ostream<Char, Traits>& os, const cstring<N>& str) {
